...
contract GpgEncryption {
    ...
    event MessageEncrypted(address sender, address recipient, uint256 messageId);
 ...
 /// @notice allows an address to set/assign its gpg private key
    ...
    function encryptMessageForAddress(string memory _messageURL, address _recipient) public{
        require(userPublicGpgKeys[msg.sender].length != 0, "You need to first assign your gpg public key to your address, this key will be requested by recipient to validate its the actual recipient of the message");
    ...
    function decryptMessage(bytes32 _senderPublicKey, uint256 _messageId) public view returns(string memory) {
        ...
        require(idToEncryptedMessage[_messageId].recipient == msg.sender, "check that you are the recipient of the message with this");
        require(userPublicGpgKeys[idToEncryptedMessage[_messageId].sender] == _senderPublicKey, "you dont have the correct key to decrpyt this message, contact the sender");
